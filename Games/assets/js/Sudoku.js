(function hideVars() {
    ...
    startNew.onclick = function() {
      if (document.getElementById('makeSure').checked) { 
        var makeSure = window.confirm('Bạn có chắc chắn làm mới lại trò chơi?');
        if (!makeSure) {
         return
        }
      }
    ...
    checkBtn.onclick = function() {
      ...
      if (checkBoard(viruateBoardBasedOnUI(board))) {
        tmOut = 600;
        document.body.style.borderTop = '30px solid red';
        document.body.style.borderLeft = '30px solid red';
        document.body.style.borderRight = '30px solid red';
        document.getElementsByTagName('h1')[0].style.color = 'red';
      } else {
        document.body.style.borderTop = '30px solid green';
        document.body.style.borderLeft = '30px solid green';
